...
class TimedPPO(PPO):
    ...
    def train(self):
        ...
        try:
            ...
            self.update_calls += 1
            if getattr(self, 'learning_log_path', None):
                values={k:float(v) for k,v in self.logger.name_to_value.items() if k.startswith('train/')}
                with (ROOT/self.learning_log_path).open('a') as stream:
                    stream.write(json.dumps({'update_call':self.update_calls,'model_decisions':self.num_timesteps,**values})+'\n')
...
class Budget(BaseCallback):
    ...
    def _on_step(self):
        self.trace.write(json.dumps({'model_decision': self.num_timesteps,
                                    ...
                                    **{k:v for k,v in self.locals['infos'][0].items() if k!='terminal_observation'}},
                                   default=lambda x: x.tolist() if hasattr(x,'tolist') else str(x)) + '\n')
        if time.perf_counter()-self.last_report>10:
            self.last_report=time.perf_counter()
            write_json(Path(self.trace.name).parent.parent/'progress.json',
                       {'at':now(),'phase':'training','model_decisions':self.num_timesteps,
                        'rollout_seconds':self.model.rollout_seconds,'learning_seconds':self.model.update_seconds,
                        'update_calls':self.model.update_calls,'last_episode_progress':self.locals['infos'][0]['progress_pixels']})
...
def run(args):
    ...
    manifest = {'status': 'running', 'at': now(), 'session_id': args.session, 'pid': __import__('os').getpid(), 'config': config,
    ...
    def checkpoint(label):
        ...
        # No environment or ROM is contained in the SB3 ZIP; its env is excluded by SB3.
    ...
    def evaluate(label, path=None, seeds=None, record=True, max_seconds=120):
        ...
        cmd = [sys.executable, '-m', 'smb3_rl.evaluate', '--config', str(run/'config.json'),
    ...
    def eval_stage(stage, path, seeds=None):
        stage['evaluation'] = evaluate(stage['id'], path, seeds, max_seconds=90 if args.pilot else args.eval_seconds)
    ...
    try:
        ...
        model.learning_log_path=str((run/'logs/learning-updates.jsonl').relative_to(ROOT))
        optimizer_steps = [0]
        ...
        manifest['run_right_baseline'] = evaluate('hold-run-right', max_seconds=90 if args.pilot else args.eval_seconds)
        if args.pilot:
